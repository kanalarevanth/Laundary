...
import {getToken, getNameinls} from '../utils/authoperations';
function Orders() {
        ...
        <div classname="orderscontainer"> 
            ...
            <Modal isOpen={alertmodalstatus} className="rrforcancelorder">
                <div className="rrAlertheader"> 
                    <p>Alert</p>
                    <p onClick={alertfunction} className="rrmodalexit">X</p>
                </div>
                <div className="rrAlertmsg">
                    <p className="rralertsimages"><i class="fas fa-exclamation-triangle"></i></p><p className="rralertmsgpara">Are you sure want to cancel the</p>
                    <p className="rrorderno">order No: <span>ID_{cancel}</span></p> 
                </div>
            ...
            <Modal isOpen={sidemodalopen} className="rrordermodal">
                <div className="rrordermodalheads">
                    <p>Summary</p>
                    <p onClick={sidemodelfunction} className="rrmodalexit">X</p>
                </div>
                ...
                <div className="rrmodalorderdetails">
                    ...
                    <tabel className="rrmodaltable">
                        {selecteditems.length !== 0 && selecteditems.map(function(post, index){
                            return (
                            <div key={post._id}>
                                {post.productlist.map(ord => {
                                    return (
                                        ...
                                        <td>{ord.pack && "wash"},{ord.pack && "Iron"}, {ord.pack && "fold"}, {ord.pack && "packing"}</td>
                                        <td>{ord.quantity}x{ord.price/ord.quantity}</td> 
                                        <td>{ord.price}</td> 
                                        </tr>}
                                        </div>
                                    )
            ...
            <div className="orderhead">
                ...
                <div className="orderrightbutton">
                    ...
                    <h2 className="orderrightname">{getNameinls()}</h2>
                </div>
                </div>
            </div>
            ...
            <div className="ordersbody">
                <div className="ordersidebar">
                    ...
                    <div className="ordermore">
                        <Link to="/orderpage"><img src={more} alt="more"/></Link> 
                    </div>
                ...
                <div className="orderbodytop">
                    <div className="OCS">
                        ...
                        <div className="rightorderbutton">
                           <Link to="/orderPage"> <button>Create</button></Link>
                        </div> 
                        <div className="rightordersearch">
                            <div className="orderdivsearch">
                            <img src={search} alt="search"/><input type="text" className="rrorderinputs"></input>
                            </div>
                        </div>
                    </div>
                    {/* Table */}
                ...
                {status && posts.map(function(post, index){
                    return (
                        <tr key={post._id}> 
                            <td>ID_{(post._id)}</td> 
                            ...
                            {post.status === "Order Canceled" && <td className="rrOrdercancel">{post.status}</td>}
                            {post.status !== "Order Canceled" && <td>{post.status}</td>} 
                            {post.status !== "Order Canceled" && <td className="rrOrdercancel" onClick={() => cancelfunction(post._id)}>Cancel order</td>}
                            {post.status === "Order Canceled"  && <td></td>} 
                            <td className="rrmodalexit"><i onClick={() => onclickview(post._id)} class="fa fa-eye"></i></td>
                        </tr>
                        )
                    }
                      )} 
